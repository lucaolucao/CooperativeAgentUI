...
function ItemImage({
  ...
  <img
    ...
    //src={`${import.meta.env.BASE_URL}items/${scenario}/${item.image}`}
    alt={item.name}
    onError={() => setFailed(true)}
    draggable="false"
  />
...
function App() {
  ...
  function handleExport() {
    ...
    const result = {
      ...
      scenario: scenario,                       // Moon / Desert / Ocean / Winter
      scenarioTtile: scenarios[scenario].title, // Moon Survival / Desert Survival
      ...
      ranking:
        ranking.map(
          (itemId, index) => {
            ...
            const item =
              itemId
                ? getItem(ITEMS, itemId) //getItem(itemId)
                : null;
